...
with open('./lookup_failures', 'w') as lookup_failures:
    ...
    with open(latest, newline=None, errors='replace') as csvfile:
      ...
      for line in reader:
        ...
        if reader.line_num == 1:
          ...
          try:
            src_repeatable = repeatable[(int(row.src_course_id), (row.src_offer_nbr))]
          except KeyError:
            cursor.execute(f'select repeatable from cuny_courses where course_id = '
                           f'{src_course_id} and offer_nbr = {src_offer_nbr}')
            if cursor.rowcount != 1:
              print(f'Repeatable lookup for {src_course_id:06}.{src_offer_nbr}'
                    f' returned {cursor.rowcount} rows: {row.src_institution} '
                    f'{row.src_subject}-{src_catalog_nbr}', file=lookup_failures)
              src_repeatable = None
            else:
              src_repeatable = cursor.fetchone().repeatable
